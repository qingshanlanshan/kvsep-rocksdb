...
Status BlobFileBuilder::Add(const Slice& key, const Slice& value,
                            std::string* blob_index, AddOptions options) {
  assert(blob_index);
  assert(blob_index->empty());

  if (options == AddOptions::kForceKeepValue) {
    return Status::OK();
  }

  if (options == AddOptions::kDefault && value.size() < min_blob_size_) {
    return Status::OK();
  }

  if (options != AddOptions::kForceExtractValue) {
    // TBD
  }
